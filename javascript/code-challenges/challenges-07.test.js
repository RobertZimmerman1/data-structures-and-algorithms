...
const howMuchPencil = (str) => {
  ...
  for (let i = -1; i <= str.length - 1; i++) {
    ...
    let shortenedString = splitString.slice(i + 1);
    let rejoinedString = shortenedString.join('');
    result.push(rejoinedString);
  }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    ingred.push(recipe.ingredients[i].split(" ").slice(2).join(" "));
  }
  return ingred;
};
